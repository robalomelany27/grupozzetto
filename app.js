...
¿Podrían contactarme para más información?`;
    ...
    function shareCalc(){
      const P = params();
      const text = `Plan ${fmt(P.V)} · Cuota ${fmt(P.total)} (sin sello) · Cuota pura ${fmt(P.cuotaPura)} · Adm+IVA ${fmt(P.adminTotalCuota)} · Seguro ${fmt(P.seguro)} · IDC ${fmt(P.idc)}.`;
      if(navigator.share){
        navigator.share({title:'Plan', text}).catch(()=>{});
      }else{
